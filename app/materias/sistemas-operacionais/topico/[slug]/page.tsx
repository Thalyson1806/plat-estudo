...
import { getSoTopicBySlug, getSoPrevNext } from '@/data/sistemas-operacionais';
import { CodeBlock } from '@/components/CodeBlock';
import { QuizCard } from '@/components/QuizCard';

const MODULE = '/materias/sistemas-operacionais';

export default function SoTopicPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = use(params);
  const topic = getSoTopicBySlug(slug);
  if (!topic) notFound();

  const { prev, next } = getSoPrevNext(slug);
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-10">
      ...
      <nav className="flex items-center gap-2 text-sm mb-8" style={{ color: 'var(--text-muted)' }}>
        ...
        <Link href={`${MODULE}/topicos`} style={{ color: 'var(--text-muted)' }}>Sistemas Operacionais</Link>
        <ChevronRight size={14} />
        ...
        {prev ? (
          ...
          >
            ...
            <div>
              ...
              <div className="font-medium text-sm">🖥️ Sistemas Operacionais</div>
            </div>
          </Link>
        )}
        ...
        {next ? (
          ...
          >
            <div>
              ...
              <div className="font-medium text-sm">🖥️ Ver todos os tópicos</div>
            </div>
            <ChevronRight size={18} style={{ color: 'var(--accent)' }} />
          </Link>
        )}
